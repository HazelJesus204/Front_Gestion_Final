...
{/* Transformacion a csv*/} 
  const exportarParticipantes = async (evento) => {
  try {
    const res = await axios.get(`http://localhost:8080/api/inscripciones/evento/${evento.id}/participantes`);
    const datos = res.data;
    if (datos.length === 0) { alert("Este evento no tiene participantes aún."); return; }
    const encabezado = "ID,Username,Fecha Inscripcion\n";
    const filas = datos.map(p => `${p.id},${p.username},${p.fechaInscripcion}`).join("\n");
    const blob = new Blob([encabezado + filas], { type: "text/csv;charset=utf-8;" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = `participantes_${evento.nombre}.csv`;
    a.click();
...
//cloudery
  ...
  {esMiEvento && (
    <button
      ...
      onClick={() => exportarParticipantes(evento)}
      style={{ backgroundColor: "#22c55e", color: "white" }}>
      <Users size={16} /> Exportar participantes
    </button>
  )}
                    <button
